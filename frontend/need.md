...
> 最后更新：2026-06-05（当日修订：清理过期状态标记、更新实现进度）
...
| # | 缺口 | 涉及位置 | 状态 |
|---|------|---------|------|
| 1 | 前端认证 UI（AuthModal + NavBar + 路由守卫） | `AuthModal.vue`, `NavBar.vue`, `router/index.ts` | 🟡 |
| 2 | 首页未登录欢迎状态 | `HomePage.vue` | 🟡 |
| 3 | 编辑页全宽布局 | `App.vue` | 🟡 |
| 4 | 编辑页分隔条可拖拽 | `EditorPage.vue` | 🟡 |
| 5 | 编辑页下载按钮（Source 左, PDF 右, 解除 editId 限制） | `EditorPage.vue` | 🟡 |
| 6 | `POST /compile-download`（新建文章时编译下载） | `backend/routes/compile.py` | 🟡 |
| 7 | 种子数据更新（username + password） | `seed.py` | 🟡 |
