...
```
        ...
        ▼           ▼           ▼           ▼           ▼           ▼
   ┌─────────┐ ┌─────────┐ ┌─────────┐ ┌─────────┐ ┌─────────┐ ┌─────────┐
   │  auth   │ │articles │ │ reviews │ │  users  │ │  pool   │ │  feed   │
   │  注册    │ │ CRUD+   │ │ 评审    │ │ 关注    │ │ 沉淀池  │ │ 动态    │
   │  登录    │ │ Git+    │ │ 讨论串  │ │ 取关    │ │         │ │         │
   │  me     │ │ Bundle  │ │         │ │         │ │         │ │         │
   └────┬────┘ └────┬────┘ └────┬────┘ └────┬────┘ └────┬────┘ └────┬────┘
        │           │           │           │           │           │
...
关键规则：
- **main.py 是唯一中心**——所有路由通过它注册，不互相引用
- **deps.py 是共享枢纽**——所有 route 通过 `Depends()` 注入，不自己管理 DB/JWT
- **routes 之间不互相调用**——一个 route 需要其他功能时，调 core，不调另一个 route
- **schemas 是纯数据契约**——没有业务逻辑，被 routes 和 deps 引用
