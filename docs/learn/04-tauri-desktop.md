...
```
                         ┌──────────────────┐
                         │    main.rs       │  ← 中心：打开 SQLite、注册 62 个命令
                         │  AppState {      │
                         │    db: Mutex<...>│
                         │  }               │
                         └────────┬─────────┘
        ...
        ▼             ▼           ▼           ▼           ▼             ▼
  ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐
  │commands  │ │local_auth│ │local_    │ │local_git │ │  db.rs   │ │ error.rs │
  │  .rs     │ │  .rs     │ │store.rs  │ │  .rs     │ │ 迁移 v10 │ │ AppError │
  │ 62 个 IPC│ │ bcrypt   │ │ SQLite   │ │ git CLI  │ │          │ │ 5 变体   │
  │ 命令入口  │ │ 会话管理  │ │ CRUD     │ │ 封装      │ │          │ │          │
  └────┬─────┘ └────┬─────┘ └────┬─────┘ └────┬─────┘ └────┬─────┘ └────┬─────┘
       │            │            │            │            │            │
       └────────────┴────────────┴────────────┴────────────┴────────────┘
                                     │ 模块之间不互相依赖
                                     │ 每个模块只依赖 AppState 和标准库
                                     ▼
                            ┌──────────────────┐
                            │   前端 JS 层      │  ← 通过 invoke() 调用
                            │  useTauri.ts     │
                            │  useUserStore    │
                            └──────────────────┘
```
...
关键规则：
- **main.rs 是唯一入口**——所有 IPC 命令在这里注册，前端通过 `invoke()` 调用
- **模块之间零依赖**——`local_auth` 不调 `local_store`，`local_git` 不调 `db`
- **AppState 只传 SQLite 连接**——不传业务对象、不传配置
- **Git 操作不碰数据库**——直接读写文件系统（~/.peerpedia/articles/）
