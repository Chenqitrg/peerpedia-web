...
```
Phase 1 + 1.5（Tauri Desktop — MVP + 打磨）
┌─────────────────────────────────────────────────────────┐
...
详细工程计划见 [`docs/plan_reshape.md`](plan_reshape.md)。
...
|-----------|-------------|
| 索引服务器 | 内容哈希 → 对等节点地址 |
| 对等客户端 | 嵌入 Tauri 桌面，缓存并提供文章 |
| 内容哈希 | SHA-256 → `peerpedia://<哈希>` |
| 同步引擎 | 后台拉取/推送热门文章 |
...
*最后更新: 2026-06-09 · 120 后端测试 · 327 前端测试 · 16 Rust 测试 · 9 个 DB 实体*
