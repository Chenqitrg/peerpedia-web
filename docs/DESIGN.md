...
| 编译 | Markdown: client-side (marked + KaTeX). Typst: Tauri sidecar CLI (Slice 2) | Markdown: client-side (marked + KaTeX). Typst: on-going |
...
**编译架构：** 服务器不编译。Markdown → HTML + KaTeX 在客户端完成（`frontend/src/utils/markdown.ts`）。Typst 编译由 Tauri sidecar 本地 Typst CLI 完成（Slice 2），Web 版 on-going。
...
**编译预览：** Markdown 客户端渲染（marked.js → HTML → KaTeX）。Typst 预览依赖 Tauri sidecar（Slice 2），Web 版 on-going。
...
> ⚠️ **Deprecated (2026-06-06):** `/compile-preview` 和 `/compile-download` 已废弃。Markdown 编译已迁移至客户端（`frontend/src/utils/markdown.ts`，marked + KaTeX）。Typst 编译由 Tauri sidecar 本地完成（Slice 2）。这两个端点保留仅用于向后兼容，新代码不得使用，未来版本将移除。

| 方法 | 端点 | 认证 | 说明 |
|------|------|------|------|
| POST | `/compile-preview` | 无 | ⛔ Deprecated — body: `{content, format}` → HTML/SVG |
| POST | `/compile-download` | 无 | ⛔ Deprecated — body: `{content, format}` → 文件下载 |
| GET | `/search?q=` | 无 | 全文搜索 |
