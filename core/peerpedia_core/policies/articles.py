...
def assert_can_read_article(
    ...
    if _is_author(db, article_id, current_user):
        return a
    raise NotAuthorized("Article is private")
...
def assert_can_download_repo(
    ...
    if _is_author(db, article_id, current_user):
        return a
    raise NotAuthorized("Repo download not available for this article")
...
def _assert_is_author(
    ...
    if not _is_author(db, article_id, current_user):
        raise NotAuthorized(f"Only authors can {action} this article")
    return a
...
def assert_can_fork_article(
    ...
    if a.status not in FORKABLE_STATUSES:
        raise NotAuthorized("Only published articles can be forked")

    existing = get_article_by_fork_and_author(
        db, forked_from=article_id, author_id=current_user.id,
    )
    if existing is not None:
        raise Conflict("Already forked this article")
...
def require_self_review_for_publish(
    ...
    if existing is None:
        raise BadRequest("self_review is required before publishing")
