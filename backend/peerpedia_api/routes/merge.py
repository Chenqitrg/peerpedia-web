...
def api_list_merges(article_id: str, db: Session = Depends(deps.get_db)):
    ...
    return {
        "proposals": [
            {
                "id": p.id,
                "fork_article_id": p.fork_article_id,
                "target_article_id": p.target_article_id,
...
def api_accept_merge(
    article_id: str, proposal_id: str, current_user: User = Depends(deps.require_user), db: Session = Depends(deps.get_db)
):
