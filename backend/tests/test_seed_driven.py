...
class TestSeedSmoke:
    ...
    def test_multi_author_articles_exist(self, client):
        ...
        multi = [a for a in articles if len(get_article_authors(s, a.id)) >= 2]
        s.close()
        engine.dispose()
...
class TestForkMergeFlow:
    ...
    def test_fork_then_merge(self, client):
        ...
        (get_article_authors(s, parent.id) or [None])[0] if parent else None
        resp2 = client.post(
...
class TestCitationGraph:
    ...
    def test_get_citations_for_article(self, client):
        ...
        a = s.query(Article).filter(Article.id.in_(s.query(ArticleAuthor.article_id).filter(ArticleAuthor.author_id == b.id))).first()
        s.close()
        engine.dispose()
