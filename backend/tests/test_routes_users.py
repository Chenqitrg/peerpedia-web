...
class TestFollow:
    ...
    def test_follow_nonexistent_user_returns_404(self, client, auth_header):
        ...
        a = client.post("/api/v1/users", json={
            "username": "fol_404", "password": "666666",
            "email": "f@t.com", "name": "Follower",
        }).json()
    ...
    def test_create_user_with_optional_fields(self, client):
        ...
        resp = client.post("/api/v1/users", json={
            "username": "full_user", "password": "pass123",
            "email": "full@test.com", "name": "Full User",
            "affiliation": "MIT",
    ...
    def test_update_own_profile(self, client, auth_header):
        ...
        a = client.post("/api/v1/users", json={
            "username": "self_edit", "password": "pass123",
            "email": "s@t.com", "name": "Self Editor",
        }).json()
        headers = auth_header(a["id"])
        resp = client.put(
            f"/api/v1/users/{a['id']}",
    ...
    def test_update_anonymous_name(self, client, auth_header):
        ...
        a = client.post("/api/v1/users", json={
            "username": "anon_edit", "password": "pass123",
            "email": "ae@t.com", "name": "Anon Editor",
        }).json()
        headers = auth_header(a["id"])
        resp = client.put(
            f"/api/v1/users/{a['id']}",
    ...
    def test_update_avatar_url(self, client, auth_header):
        ...
        a = client.post("/api/v1/users", json={
            "username": "avatar_edit", "password": "pass123",
            "email": "av@t.com", "name": "Avatar Editor",
        }).json()
        headers = auth_header(a["id"])
        resp = client.put(
            f"/api/v1/users/{a['id']}",
    ...
    def test_update_contact(self, client, auth_header):
        ...
        a = client.post("/api/v1/users", json={
            "username": "contact_edit", "password": "pass123",
            "email": "ce@t.com", "name": "Contact Editor",
        }).json()
        headers = auth_header(a["id"])
        resp = client.put(
            f"/api/v1/users/{a['id']}",
